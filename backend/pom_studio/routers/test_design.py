...
import traceback
from pom_studio.services.test_design import (
    enhanced_rag_helper,
    image_helper,
...
def validate_and_filter_test_cases(test_cases: List[Dict], user_story: str) -> List[Dict]:
    try:
        from pom_studio.services.test_design.optimized_prompt_engineering import OptimizedPromptEngineer as PromptEngineer
...
async def generate_test_cases(
    ...
    try:
            ...
            for attachment in attachments:
                ...
                try:
                    if ext in ['png', 'jpg', 'jpeg', 'bmp', 'tiff', 'webp']:
                        ...
                        try:
                            from pom_studio.services.test_design.image_helper import get_detailed_image_analysis, create_image_test_context
        ...
        try:
            from pom_studio.services.test_design.optimized_prompt_engineering import OptimizedPromptEngineer
...
async def submit_feedback(data: Dict[str, Any]):
    try:
        ...
        landing_feedback_text = user_feedback or "No text feedback provided."
        ...
        try:
            ...
            landing_feedback_text = full_message
        ...
        # 3. Also publish into landing page feedback stream (agent-eval feedback table)
        try:
            from agent_database import save_feedback

            save_feedback(
                rating=max(1, min(int(quality_score), 5)),
                suggestion=f"[Playwright POM - Test Design] {landing_feedback_text}",
                app_id=None,
            )
            print("✅ Published Test Design feedback to landing feedback stream")
        except Exception as landing_e:
            print(f"⚠️ Failed to publish to landing feedback stream: {landing_e}")
            # Keep primary endpoint success even if cross-stream publish fails
