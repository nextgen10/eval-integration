...
import {
    Play, Save, Download, Copy, FolderOpen, Trash2,
    RefreshCw, Check, CheckCircle, X, Terminal,
    Layers, Search, AlertCircle, XCircle, Box as BoxIcon,
    Wand2, Sparkles, Cpu, Info, FileCode, Eye,
    MessageSquare, Bot, User, ChevronDown,
    Square, Video, Folder, Upload, FolderInput, Clock, ChevronRight,
    Circle, Rocket, Scissors
} from 'lucide-react';
import { AIChatWindow } from '../UI/AIChatWindow';
import { Card } from '../UI/Card';
import { Button } from '../UI/Button';
import { alpha, Box, Typography, Paper, Menu, MenuItem, Divider, Tooltip } from '@mui/material';
        ...
        <>
            ...
            {showRunRawModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => !runningRaw && setShowRunRawModal(false)}>
                    <div
                        className={`w-[800px] h-[70vh] flex flex-col rounded-xl border shadow-2xl overflow-hidden ${isDark ? 'bg-[#1e1e1e] border-gray-800' : 'bg-white border-gray-300'}`}
            ...
            {showLogsModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setShowLogsModal(false)}>
                    <div
                        className={`w-[800px] h-[80vh] flex flex-col rounded-xl border shadow-2xl overflow-hidden ${isDark ? 'bg-[#1e1e1e] border-gray-800' : 'bg-white border-gray-300'}`}
                        onClick={(e) => e.stopPropagation()}
                    >
                        {/* Header */}
                        <div
                        ...
                        >
                            <div className="flex items-center gap-3" style={{ marginLeft: 10 }}>
                                ...
                                {runningTest && (
                                    <div className={`flex items-center gap-2 px-2 py-1 rounded-full border text-xs font-semibold ${isDark ? 'bg-blue-500/10 border-blue-500/20 text-blue-400' : 'bg-blue-50 border-blue-200 text-blue-700'}`}>
                            ...
                            <div className="flex items-center gap-2">
                                {testResult && (
                                    <button
                                        ...
                                        className={`p-1.5 rounded-md transition-colors ${copied ? 'text-green-500' : (isDark ? 'text-blue-400 hover:bg-white/10' : 'text-blue-600 hover:bg-gray-200')}`}
                                    >
                        ...
                        >
                            ...
                            {!runningTest && testResult && (
                                <div className="space-y-4">
                                    ...
                                    {testResult.output && (
                                        <div>
                                            <div className={`text-xs mb-2 uppercase tracking-wider ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Output:</div>
            ...
            {showPromptsModal && aiPrompts && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setShowPromptsModal(false)}>
                    <div
                        ...
                        onClick={(e) => e.stopPropagation()}
                    >
                        <div className={`flex items-center justify-between px-4 border-b ${isDark ? 'border-gray-800 bg-[#252526]' : 'border-gray-300 bg-gray-50'}`} style={{ paddingTop: 16, paddingBottom: 16, minHeight: 72 }}>
                            <div className="flex items-center gap-2.5" style={{ marginLeft: 24 }}>
                                <Sparkles className={isDark ? 'text-purple-400' : 'text-purple-600'} size={18} />
                        ...
                        <div className={`flex-1 overflow-y-auto custom-scrollbar space-y-4 ${isDark ? 'bg-[#1e1e1e]' : 'bg-white'}`} style={{ padding: "20px" }}>
                            {/* System Prompt */}
                            <div style={{ margin: 0, marginLeft: 10 }}>
                                <h4 className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-gray-500' : 'text-gray-600'}`} style={{ margin: 0, marginBottom: 10 }}>System Prompt</h4>
                            ...
                            <div style={{ margin: 0, marginTop: 14, marginLeft: 10 }}>
                                <h4 className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-gray-500' : 'text-gray-600'}`} style={{ margin: 0, marginBottom: 10 }}>User Prompt</h4>
            ...
            <Box sx={{ height: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                {/* Compact status bar — replaces the standalone app's 70px header */}
                {status && (
                    <Box sx={{ px: 2, py: 0.75, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 1, borderBottom: '1px solid', borderColor: 'divider', bgcolor: (theme) => status.severity === 'success' ? alpha(theme.palette.success.main, 0.08) : alpha(theme.palette.error.main, 0.08) }}>
                        {status.severity === 'success' ? <CheckCircle size={14} /> : <AlertCircle size={14} />}
                        <Typography variant="caption" color={status.severity === 'success' ? 'success.main' : 'error.main'}>{status.message}</Typography>
                    </Box>
                )}
                        ...
                        <div className={`relative w-[600px] h-[70vh] flex flex-col rounded-xl border shadow-2xl overflow-hidden ${isDark ? 'bg-[#1e1e1e] border-gray-800' : 'bg-white border-gray-300'}`}>
                            ...
                            {confirmDeleteFile && (
                                <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-[2px] rounded-xl">
                                    <div className={`w-[320px] p-5 rounded-xl shadow-2xl border transform transition-all scale-100 ${isDark ? 'bg-[#252526] border-gray-800' : 'bg-white border-gray-300'}`}>
                                        ...
                                        <p className={`text-xs mb-4 leading-relaxed ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                                            Are you sure you want to delete <span className="font-mono text-blue-500 break-all">{confirmDeleteFile}</span>?
                            ...
                            {showDeleteConfirmOverlay && selectedItemsToDelete.size > 0 && (
                                <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-[2px] rounded-xl">
                                    <div className={`w-[320px] p-5 rounded-xl shadow-2xl border transform transition-all scale-100 ${isDark ? 'bg-[#252526] border-gray-800' : 'bg-white border-gray-300'}`}>
                                        ...
                                        <p className={`text-xs mb-4 leading-relaxed ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                                            Are you sure you want to delete <span className="font-bold text-blue-500">{selectedFileCount}</span> file(s)?
                                ...
                                <div className="relative group">
                                    ...
                                    <input
                                        ...
                                        className={`w-full pl-9 pr-9 py-2 rounded-lg text-sm border transition-all outline-none 
                                            ${isDark
                                                ? 'bg-black/20 border-gray-800 focus:border-blue-500/50 text-gray-200 placeholder-gray-600'
                ...
                {previewFile && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                        <div className={`w-[800px] h-[80vh] flex flex-col rounded-xl border shadow-2xl overflow-hidden ${isDark ? 'bg-[#1e1e1e] border-gray-800' : 'bg-white border-gray-300'}`}>
                            ...
                            {/* Editor Content */}
                            <div className={`flex-1 relative group min-h-0 ${isDark ? 'bg-[#1e1e1e]' : 'bg-white'}`}>
                                ...
                                <textarea
                                    className={`absolute inset-0 w-full h-full resize-none bg-transparent text-transparent caret-blue-500 focus:outline-none z-10 font-mono text-sm leading-relaxed overflow-auto scrollbar-thin scrollbar-track-transparent ${isDark ? 'scrollbar-thumb-gray-700/50 hover:scrollbar-thumb-gray-600' : 'scrollbar-thumb-gray-200 hover:scrollbar-thumb-gray-300'}`}
                                    style={{
                                        fontFamily: 'monospace',
                                        fontSize: '0.875rem',
                                        lineHeight: '1.5',
                    ...
                    <div className="grid gap-4" style={{ flex: 1, minHeight: 0, overflow: 'hidden', gridTemplateColumns: 'minmax(0, 3.2fr) minmax(0, 1.8fr)' }}>
                        ...
                        <div className="flex flex-col min-h-0 overflow-hidden">
                            <div className={`flex-1 flex flex-col rounded-xl border shadow-2xl overflow-hidden transition-colors duration-300 ${isDark ? 'bg-[#1e1e1e] border-gray-800 ring-white/5' : 'bg-white border-gray-300 ring-black/5'} ring-1`}>
                                {/* Window Header */}
                                <div className={`relative h-10 flex items-center justify-between pl-6 pr-8 shrink-0 transition-colors duration-300 ${isDark ? 'bg-[#252526]' : 'bg-gray-50'}`}>
                                    ...
                                    <div className="relative z-10 h-full flex items-center gap-2">
                                        ...
                                        <div className={`flex items-center px-4 py-1 rounded-t-lg text-xs font-medium border-t-2 relative transition-all duration-300 font-sans ${isDark ? 'bg-[#1e1e1e] text-gray-200 border-t-blue-500 border-x border-x-[#333]' : 'bg-white text-gray-700 border-t-blue-500 border-x border-x-gray-200'}`}>
                                            <FileCode size={13} className={`mr-2 shrink-0 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} />
                                            ...
                                            <div className={`absolute bottom-[-2px] left-0 right-0 h-[2px] ${isDark ? 'bg-[#1e1e1e]' : 'bg-white'}`} />
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-5 shrink-0 -translate-x-4">
                                        {recording && (
                                            <div className="flex items-center gap-2 px-2 py-0.5 bg-red-500/10 rounded border border-red-500/20">
                                                ...
                                                <span className={`text-[10px] font-bold tracking-wider ${isDark ? 'text-red-400' : 'text-red-600'}`}>REC</span>
                                            </div>
                                        )}
                                        {!recording && (
                                            <>
                                                ...
                                                <Tooltip title="Save As..." arrow placement="top">
                                                    <button
                                                        ...
                                                        className={`p-1.5 rounded-md transition-all ${isDark ? 'text-purple-400 hover:text-purple-300 hover:bg-purple-500/10' : 'text-purple-600 hover:text-purple-700 hover:bg-purple-50'}`}
                                                    >
                                                        <Download size={14} />
                                                    </button>
                                                </Tooltip>
                                                <Tooltip title={codeCopied ? 'Copied!' : 'Copy Code'} arrow placement="top">
                                                    <button
                                                        ...
                                                        className={`p-1.5 rounded-md transition-all ${codeCopied ? (isDark ? 'text-green-400 bg-green-500/10' : 'text-green-600 bg-green-50') : (isDark ? 'text-blue-400 hover:text-blue-300 hover:bg-blue-500/10' : 'text-blue-600 hover:text-blue-700 hover:bg-blue-50')}`}
                                                    >
                                ...
                                {/* Editor Area */}
                                <div className={`relative flex-1 group min-h-0 transition-colors duration-300 ${isDark ? 'bg-[#1e1e1e]' : 'bg-white'}`}>
                                    ...
                                    {!recording && (
                                        <textarea
                                            className={`absolute inset-0 w-full h-full bg-transparent text-sm font-mono leading-relaxed resize-none focus:outline-none caret-blue-500 overflow-auto scrollbar-thin scrollbar-track-transparent ${isDark ? 'text-transparent selection:bg-blue-500/20 scrollbar-thumb-gray-700/50 hover:scrollbar-thumb-gray-600' : 'text-transparent selection:bg-blue-500/10 scrollbar-thumb-gray-200 hover:scrollbar-thumb-gray-300'}`}
                                            style={{
                                                fontFamily: 'monospace',
                                                fontSize: '0.875rem',
                                                lineHeight: '1.5',
                        ...
                        <div className="flex flex-col gap-4 min-h-0 overflow-hidden">
                            ...
                            >
                                <div style={{ padding: '12px 16px 6px', display: 'flex', flexDirection: 'column', height: '100%' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, flexShrink: 0 }}>
                                        <Layers className={`shrink-0 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} size={16} />
                                        <h3 className={`text-[13px] font-bold truncate ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>Automation Workflow</h3>
                                    </div>

                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
                                        {/* STEP 1: Record */}
                                        <div className={`rounded-xl border transition-all duration-300 ${recording ? (isDark ? 'bg-red-500/10 border-red-500/40 shadow-[0_0_25px_rgba(239,68,68,0.3)] ring-1 ring-red-500/50 scale-[1.03]' : 'bg-red-50 border-red-300 shadow-xl ring-2 ring-red-200 transform scale-[1.03]') : (isDark ? 'bg-[#252526] border-gray-800 hover:border-white/30 shadow-xl shadow-black/50 hover:bg-[#2d2d2e] hover:scale-[1.01]' : 'bg-white border-gray-300 shadow-lg hover:border-blue-400/50 hover:shadow-2xl hover:-translate-y-1 hover:scale-[1.01]')}`} style={{ padding: 10 }}>
                                        ...
                                        {/* STEP 2: Generate */}
                                        <div className={`rounded-xl border transition-all duration-300 ${generating ? (isDark ? 'bg-purple-500/10 border-purple-500/40 shadow-[0_0_25px_rgba(168,85,247,0.3)] ring-1 ring-purple-500/50 scale-[1.03]' : 'bg-purple-50 border-purple-300 shadow-xl ring-2 ring-purple-200 transform scale-[1.03]') : (isDark ? 'bg-[#252526] border-gray-800 hover:border-white/30 shadow-xl shadow-black/50 hover:bg-[#2d2d2e] hover:scale-[1.01]' : 'bg-white border-gray-300 shadow-lg hover:border-blue-400/50 hover:shadow-2xl hover:-translate-y-1 hover:scale-[1.01]')}`} style={{ padding: 10 }}>
                                            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
                                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold border-2 ${publishSuccess || recording
                                                        ? (isDark ? 'bg-white/5 border-gray-700 text-gray-400' : 'bg-gray-200 border-gray-300 text-gray-600')
                                                        : generating
                                                            ? (isDark ? 'bg-purple-500/10 border-purple-500 text-purple-400' : 'bg-purple-500 border-purple-600 text-white')
                                                    ...
                                                    <div className="flex-1">
                                                        <span className={`text-sm font-semibold ${isDark ? 'text-purple-400' : 'text-purple-700'}`}>Generate POM</span>
                                                        <p className={`text-xs italic ${isDark ? 'text-purple-400/80' : 'text-purple-600/80'}`}>{useAI ? 'AI-powered generation' : 'AST-based generation'}</p>
                                                    </div>
                                                </div>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end', flexShrink: 0 }}>
                                                    ...
                                                    >
                                                        ...
                                                        <div className={`h-full flex items-center justify-center gap-1 px-3 rounded-full text-[10px] font-bold transition-all duration-300 ${!useAI
                                                            ? (isDark
                                                                ? 'bg-gradient-to-r from-cyan-500 to-blue-500 text-white shadow-[0_0_12px_rgba(59,130,246,0.5)]'
                                                                : 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-md shadow-blue-300')
                                                        ...
                                                        <div
                                                            className={`h-full flex items-center justify-center gap-1 px-3 rounded-full text-[10px] font-bold transition-all duration-300 ${useAI
                                                                ? (isDark
                                                                    ? 'bg-gradient-to-r from-violet-500 via-purple-500 to-pink-500 text-white shadow-[0_0_12px_rgba(168,85,247,0.5)]'
                                                                    : 'bg-gradient-to-r from-violet-500 via-purple-500 to-pink-500 text-white shadow-md shadow-purple-300')
                                                                : (isDark ? 'text-gray-500 hover:text-gray-400' : 'text-gray-500 hover:text-gray-600')
                                                                }`}
                                                    ...
                                                    {useAI && (
                                                        <button
                                                            ...
                                                            className={`p-1.5 rounded-md transition-colors ${isDark ? 'hover:bg-white/10 text-gray-400 hover:text-purple-300' : 'hover:bg-gray-200 text-gray-400 hover:text-purple-600'}`}
                                        ...
                                        {/* STEP 3: Test */}
                                        <div className={`rounded-xl border transition-all duration-300 ${runningTest ? (isDark ? 'bg-blue-500/10 border-blue-500/40 shadow-[0_0_25px_rgba(59,130,246,0.3)] ring-1 ring-blue-500/50 scale-[1.03]' : 'bg-blue-50 border-blue-300 shadow-xl ring-2 ring-blue-200 transform scale-[1.03]') : (isDark ? 'bg-[#252526] border-gray-800 hover:border-white/30 shadow-xl shadow-black/50 hover:bg-[#2d2d2e] hover:scale-[1.01]' : 'bg-white border-gray-300 shadow-lg hover:border-blue-400/50 hover:shadow-2xl hover:-translate-y-1 hover:scale-[1.01]')}`} style={{ padding: 10 }}>
                                            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
                                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold border-2 ${publishSuccess || recording
                                                        ? (isDark ? 'bg-white/5 border-gray-700 text-gray-400' : 'bg-gray-200 border-gray-300 text-gray-600')
                                                        : runningTest
                                                            ? (isDark ? 'bg-blue-500/10 border-blue-500 text-blue-400' : 'bg-blue-500 border-blue-600 text-white')
                                                            : testResult
                                                    ...
                                                    <div className="flex-1">
                                                        <span className={`text-sm font-semibold ${isDark ? 'text-blue-400' : 'text-blue-700'}`}>Run Tests</span>
                                                        <p className={`text-xs italic ${isDark ? 'text-blue-400/80' : 'text-blue-600/80'}`}>Execute latest generated POM tests</p>
                                                    </div>
                                                </div>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end', flexShrink: 0 }}>
                                        ...
                                        {/* STEP 4: Publish */}
                                        <div className={`rounded-xl border transition-all duration-300 ${isDark ? 'bg-[#252526] border-gray-800 hover:border-white/30 shadow-xl shadow-black/50 hover:bg-[#2d2d2e] hover:scale-[1.01]' : 'bg-white border-gray-300 shadow-lg hover:border-blue-400/50 hover:shadow-2xl hover:-translate-y-1 hover:scale-[1.01]'}`} style={{ padding: 10 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
                                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold border-2 ${recording
                                                        ? (isDark ? 'bg-white/5 border-gray-700 text-gray-400' : 'bg-gray-200 border-gray-300 text-gray-600')
                                                        : publishSuccess
                                                            ? (isDark ? 'bg-green-500/10 border-green-500 text-green-400' : 'bg-green-500 border-green-600 text-white')
                                                            : (isDark ? 'bg-white/5 border-gray-700 text-gray-400' : 'bg-gray-200 border-gray-300 text-gray-600')
                            ...
                            <div className={`flex-1 flex flex-col rounded-xl border shadow-lg overflow-hidden min-h-0 transition-colors duration-300 ${isDark ? 'bg-[#1e1e1e] border-gray-800' : 'bg-white border-gray-300'}`}>
                                {/* Terminal Header / Tabs */}
                                <div className={`py-2.5 border-b flex justify-between items-center shrink-0 transition-colors duration-300 ${isDark ? 'bg-[#252526] border-gray-800' : 'bg-gray-50 border-gray-300'}`} style={{ paddingLeft: 22, paddingRight: 22 }}>
                                ...
                                <div className="flex-1 overflow-y-auto font-mono text-xs custom-scrollbar" style={{ paddingLeft: 22, paddingRight: 22, paddingTop: 16, paddingBottom: 16 }}>
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="space-y-2">
                                                            {output.files?.map((f, i) => (
                                                                ...
                                                                >
                                                                    <FileCode size={14} className={isDark ? 'text-blue-400' : 'text-blue-500'} />
                                                                    <span className={`transition-colors font-mono text-[11px] ${isDark ? 'text-gray-300 group-hover:text-white' : 'text-gray-700 group-hover:text-black'}`} style={{ lineHeight: 1.35 }}>{f}</span>
                                                                    <div className={`flex items-center gap-1 ${isDark ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-700'}`} style={{ marginLeft: 6 }}>
                                                                        <Eye size={12} />
                                                                    </div>
                                                                    <div style={{ flex: 1 }} />
                                                                </div>
                                    ...
                                    {activeTerminalTab === 'test' && (
                                        <>
                                            ...
                                            {runningTest && (
                                                <div className="space-y-3">
                                                    <div className={`flex items-center gap-2 mb-3 text-xs font-semibold ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
                                            ...
                                            {testResult && !runningTest && (
                                                <div className="space-y-3">
                                                    <div className={`flex items-center justify-between p-3 rounded border ${testResult.status === 'success' ? (isDark ? 'bg-green-500/10 border-green-500/20' : 'bg-green-50 border-green-200') : (isDark ? 'bg-red-500/10 border-red-500/20' : 'bg-red-50 border-red-200')}`}>
                                                        ...
                                                        <span className={`text-[10px] font-mono opacity-60 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Exit: {testResult.return_code}</span>
                                                    </div>

                                                    <div>
                                                        <p className={`text-[10px] uppercase font-bold tracking-wider mb-2 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Console Output</p>
                ...
                >
                    ...
                    >
                        {!showChat && (
                            <>
                                <span className={`absolute inset-0 rounded-full border ${isDark ? 'border-gray-600' : 'border-blue-600/40'} animate-[ping_2s_linear_infinite]`} />
                                <span className={`absolute inset-0 rounded-full border ${isDark ? 'border-gray-700' : 'border-blue-600/20'} animate-[ping_2s_linear_infinite_1s]`} />
                            </>
                        )}
                        {showChat ? (
